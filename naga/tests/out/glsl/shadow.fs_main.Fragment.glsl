...
void main() {
    ...
    while(true) {
        ...
        loop_init = false;
        if ((i < min(_group_0_binding_0_fs.num_lights.x, c_max_lights))) {
        } else {
            break;
        }
        {
            Light light = _group_0_binding_1_fs[i];
            float _e23 = fetch_shadow(i, (light.proj * in_.world_position));
    ...
    _fs2p_location0 = (vec4(color, 1.0) * _group_1_binding_0_fs.color);
    return;
}
